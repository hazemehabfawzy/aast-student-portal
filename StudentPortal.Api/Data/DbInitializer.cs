...
public static class DbInitializer
{
    ...
    public static async Task SyncKeycloakIds(
        AppDbContext db, 
        ...
        ILogger logger)
    {
        try
        {
            var keycloakUrl = config["Keycloak:Authority"]
                ?.Replace("/realms/student-portal", "")
                ?? "http://keycloak:8080";
            ...
            {
                logger.LogWarning("KeycloakSync: Could not get admin token");
                return;
            }
            ...
            var usersResp = await http.GetAsync(
                $"{keycloakUrl}/admin/realms/student-portal/users?max=50");

            if (!usersResp.IsSuccessStatusCode)
            {
                logger.LogWarning("KeycloakSync: Could not get users");
                return;
            }

            var usersJson = await usersResp.Content.ReadAsStringAsync();
            var users = System.Text.Json.JsonDocument.Parse(usersJson)
                ...
                .Where(u => u.Username != null && u.Id != null)
                .ToList();

            int updated = 0;

            // Map student usernames to student numbers
            ...
            var instructorMap = new Dictionary<string, string>
            {
            ...
            {
                if (user.Username == null || user.Id == null) continue;

                // Update students
                if (studentMap.TryGetValue(user.Username, out var studentNumber))
                {
                    var student = await db.Students
                        .FirstOrDefaultAsync(s => s.StudentNumber == studentNumber);
                    if (student != null && student.KeycloakId != user.Id)
                    {
                        student.KeycloakId = user.Id;
                        updated++;
                        logger.LogInformation(
                            "KeycloakSync: Updated {Username} -> {Number}: {Id}",
                            user.Username, studentNumber, user.Id[..8]);
                    }
                }

                // Update instructors
                if (instructorMap.TryGetValue(user.Username, out var instructorName))
                {
                    var instructor = await db.Instructors
                        .FirstOrDefaultAsync(i => i.FullName == instructorName);
                    if (instructor != null && instructor.KeycloakId != user.Id)
                    {
                        instructor.KeycloakId = user.Id;
                        updated++;
                        logger.LogInformation(
                            "KeycloakSync: Updated {Username} -> {Name}: {Id}",
                            user.Username, instructorName, user.Id[..8]);
                    }
                }
            }
            ...
            if (updated > 0)
            {
                await db.SaveChangesAsync();
                logger.LogInformation(
                    "KeycloakSync: Synced {Count} records", updated);
            }
            else
            {
                logger.LogInformation("KeycloakSync: All IDs already in sync");
            }
        }
        catch (Exception ex)
        {
            // Never crash the API for sync failure
            logger.LogWarning("KeycloakSync: {Message}", ex.Message);
        }
    }
}
